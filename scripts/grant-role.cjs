...
(async () => {
  ...
  if (!pk) {
    console.error('Set PRIVATE_KEY in backend/.env (owner/admin key required)');
    process.exit(1);
  }
  if (!to) {
    console.error('Usage: node scripts/grant-role.js 0xYourAddressHere');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('Granting role', role, 'to', to);
